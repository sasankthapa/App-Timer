...
exportScriptToDirectory();

/**
 * Store the scripts in a config folder in the folder rather than the snapshot so that it works.
 * Should only enable this when exported, else it should be commented out
 */
function exportScriptToDirectory(){
  var dir= './script'
  if (!fs.existsSync(dir)){
    fs.mkdirSync(dir);
  }
  if(config.name==='mac.sh'){
    var path1=config.parameters.pop();
    var shpath=config.parameters.pop();
    fs.writeFileSync('./script/'+config.name,shpath);
    fs.writeFileSync('./script/'+config.subscriptName,path1);
    config.parameters.push('./script/'+config.name)
    config.parameters.push('./script/'+config.subscriptName)
    return
  }
  var path=config.parameters.pop();
  fs.writeFileSync('./script/'+config.name,fs.readFileSync(path))
  config.parameters.push('./script/'+config.name)
}
